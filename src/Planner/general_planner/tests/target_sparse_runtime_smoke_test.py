...
try:
    ...
    while True:
        ...
        time.sleep(.1)
    subprocess.run(["rosrun", "general_planner", "target_sparse_domain_self_test"],
    ...
    runtime = start(["roslaunch", str(launch), "initial_mode:=state2state",
                     ...
                     "target_exploration_auto_workspace:=true",
                     "exploration_mission_mode:="+env.get("TEST_MISSION_MODE", "target"),
                     "target_exploration_workspace_half_extent_xy:=10.0"])
    import rospy
    ...
    # Target ignores legacy auto_workspace; coverage retains its explicitly
    # enabled bootstrap behavior.
    coverage = env.get("TEST_MISSION_MODE") == "coverage"
    assert rospy.get_param("/planner_runtime_node/box_num") == (1 if coverage else 2)
    assert rospy.get_param("/planner_runtime_node/box_0/up")[0] == (10.0 if coverage else 20.0)
    print("runtime startup / actual PlannerStatus / mission-specific workspace compatibility: PASS")
except Exception:
    print("\n".join(output))
    raise
finally:
