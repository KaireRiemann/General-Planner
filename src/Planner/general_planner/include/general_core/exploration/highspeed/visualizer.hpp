...
public:
  ...
  template <typename TrajectoryT> inline void visualize(const TrajectoryT &traj, double max_vel) {
  ...
  inline void visualizeTrajectorySamples(const std::vector<Eigen::Vector3d> &samples,
                                         const std::vector<double> &speed_ratios,
    ...
    if (!wayPointsMarker.points.empty()) {
      wayPointsPub.publish(wayPointsMarker);
    }
    ...
    for (std::size_t i = 0; i < samples.size(); ++i) {
      ...
      point.y = samples[i].y();
      point.z = samples[i].z();
      const double ratio = i < speed_ratios.size() ? speed_ratios[i] : 0.0;
      const Eigen::Vector3d color = jetColorMap(ratio);
      trajMarker.color.r = color.x();
      trajMarker.color.g = color.y();
      trajMarker.color.b = color.z();
      trajMarker.colors.push_back(trajMarker.color);
      trajMarker.points.push_back(point);
    }
    trajectoryPub.publish(trajMarker);
  ...
  inline void visualizePolytope(const std::vector<Eigen::MatrixX4d> &hPolys, bool red_edge = false) {
    ...
    for (size_t id = 0; id < hPolys.size(); id++) {
      ...
      if (!geometry_utils::enumerateVs(hPolys[id], vPoly, 1.0e-6) || vPoly.cols() < 4) {
        continue;
      }
