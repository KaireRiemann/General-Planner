...
inline Eigen::Matrix<double,3,8> trackingBrakeCoefficients(
  ...
  if (!start.allFinite() || !end.allFinite() || !std::isfinite(duration) || duration <= 0.0)
...
inline Eigen::Matrix<double,3,8> trackingBrakeCoefficients(
    const Eigen::Matrix<double,3,4>& start, double duration) {
  return trackingBrakeCoefficients(start,duration,
      start.col(0)+start.col(1)*duration/2+start.col(2)*duration*duration/12);
}
}
