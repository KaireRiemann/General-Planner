...
void FrontierManager::visfrtcluster() {
  ...
  for (auto &sf_cluster : cluster_list_) {
    ...
    if (sf_cluster->state_ == FrontierState::VISITED ||
        sf_cluster->state_ == FrontierState::BLACKLISTED ||
        sf_cluster->state_ == FrontierState::SUSPENDED ||
        !sf_cluster->is_reachable_ || sf_cluster->is_dormant_) {
      SetMarker(VizColor::BLACK, "aabb", 1.0, 0.5, aabb_marker, 1.0);
    } else if (sf_cluster->state_ == FrontierState::SELECTED) {
      SetMarker(VizColor::YELLOW, "aabb", 1.0, 0.5, aabb_marker, 1.0);
...
void FrontierManager::viz_point(PointVector &pts2viz, string topic_name) {
  ...
  if (occ_pub.getNumSubscribers() == 0U)
    return;
