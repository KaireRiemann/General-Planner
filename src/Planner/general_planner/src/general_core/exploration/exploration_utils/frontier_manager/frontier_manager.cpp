...
void FrontierManager::setTaskDomain(bool target) {
  if (target_task_domain_ == target) return;
  // Called on the serial exploration queue, between planning passes. Keep the
  // physical cell map and stable indices, but isolate each task's lifecycle.
  cluster_list_.swap(inactive_task_clusters_);
  target_task_domain_ = target;
  ++frontier_revision_;
