...
bool ExpTrajOpt::processCorridor() {
    ...
    for (long i = 0; i < sizeCorridor; i++) {
        if (!geometry_utils::enumerateVs(opt_vars.hPolytopes[i], curIV)) {
            cout << YELLOW << " -- [GeneralPlanner] in [ GcopterExpS4::processCorridor]: Failed to enumerate corridor Vs." << RESET
                 << endl;
            return false;
        }
        nv = curIV.cols();
    ...
    if (!geometry_utils::enumerateVs(opt_vars.hPolytopes.back(), curIV)) {
        cout << YELLOW << " -- [GeneralPlanner] in [ GcopterExpS4::processCorridor]: Failed to enumerate corridor Vs." <<
             RESET << endl;
        return false;
    }
...
bool ExpTrajOpt::processCorridorWithGuideTraj() {
    ...
    for (int i = 0; i < sizeCorridor; i++) {
        ...
        if (dis < 0.0 || std::isinf(dis)) {
            ...
            cout << YELLOW << " -- [GeneralPlanner] in [ GcopterExpS4::processCorridor]: Failed findInteriorDist Vs." <<
                 RESET << endl;
            return false;
        }
...
//        cout << YELLOW << " -- [GeneralPlanner] Minco corridor preprocess error." << RESET << endl;
//        success = false;
//    }
//
//    if (success && std::isinf(optimize(out_traj, cfg_.opt_accuracy))) {
//        std::cout << YELLOW << " -- [GeneralPlanner] in [ExpTrajOpt::optimize]: Optimization failed." << RESET << std::endl;
//        success = false;
//    }
//
//    if(success){
...
bool ExpTrajOpt::optimize(const StatePVAJ &headPVAJ, const StatePVAJ &tailPVAJ,
    ...
    if (success && std::isinf(optimize(out_traj, cfg_.opt_accuracy))) {
        cout << YELLOW << " -- [GeneralPlanner] Minco exp_traj opt failed." << RESET << endl;
        success = false;
    }
