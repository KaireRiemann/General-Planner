...
PX4CtrlFSM::PX4CtrlFSM(Parameter_t &param_, Controller &controller_) : param(param_), controller(controller_) /*, thrust_curve(thrust_curve_)*/
{
	...
	hover_pose.setZero();
	cmd_sequence = 0;
	takeoff_land_data.setSeq(&cmd_sequence);
	arm_data.setSeq(&cmd_sequence);
}
...
{
	//set takeoff hight
	if(takeoff_land_data.takeoff_hight > 0.0 && takeoff_land_data.takeoff_hight < 5.0) 
				param.takeoff_land.height = takeoff_land_data.takeoff_hight;
	...
	static bool do_once_hover = false;
	static bool do_once_land = false;
	static bool motor_low_speed = false;
	
	//kdkd land detect 
	static bool land_flag = false; 

	//process arm cmd 
	if(arm_data.arm_cmd != 0)
	{
		if(arm_data.arm_cmd == 1)
		{
			if(state_data.current_state.armed)
			{
				ROS_WARN("[px4ctrl] aleardy armed!!!");
			}
			else
			{
				if(toggle_offboard_mode(true)) // toggle off offboard after disarm
				{
					...
					{
						ROS_ERROR("----------arm_cmd : armed failed----------");
						px4_state = PX4_STATE::DIS_AEMED;
					}
					else
					{
						ROS_ERROR("----------arm_cmd : armed successed----------");
						px4_state = PX4_STATE::ARMED;
						// rotor_low_speed_during_land = true;
						motor_low_speed = true;
					}
				}
				else
				{
					ROS_ERROR("unable to jump to offboard mode");
				}
			}
		}
		if(arm_data.arm_cmd == -1)
		{
			...
			{
				ROS_ERROR("----------exit offboard failed----------");
			}
			else
			{
				ROS_ERROR("----------exit offboard successed----------");
				state = MANUAL_CTRL;
				px4_state = PX4_STATE::DIS_AEMED;
				motor_low_speed = false;
				cmd_data.have_cmd_data = false;
			}
		}
		arm_data.arm_cmd = 0;
	}
	...
	case MANUAL_CTRL:
	{
		...
		if (rc_data.enter_hover_mode) // Try to jump to AUTO_HOVER
		{
			...
			if(toggle_offboard_mode(true)) //进入offboard模式
			{
				motor_low_speed = false;
				state = AUTO_HOVER;
			...
			{
				ROS_ERROR("----------ENTER OFFBOARD MODE FAILED----------");
			}
		}
		else if (param.takeoff_land.enable && takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::TAKEOFF) // Try to jump to AUTO_TAKEOFF
		{
			...
			if (!param.takeoff_land.no_RC && rc_is_received(now_time)) // Check this only if no_RC is disable and RC is connected.
			{
			...
			if(toggle_offboard_mode(true))//// toggle on offboard before arm
			{
				for (int i = 0; i < 10 && ros::ok(); ++i) // wait for 0.1 seconds to allow mode change by FMU // mark
				{
					ros::Duration(0.01).sleep();
	...
	case AUTO_HOVER:
	{
		...
		else if (rc_data.is_command_mode && cmd_is_received(now_time) && cmd_data.have_cmd_data)
		{
			if (state_data.current_state.mode == "OFFBOARD")
			{
				duration_time_hover_to_cmd = ros::Time::now();
				state = CMD_CTRL;
				des = get_cmd_des();
		...
		else if (takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::LAND)
		{
			...
			state = AUTO_LAND;
			set_start_pose_for_takeoff_land(odom_data);
			std::cout << "takeoff start pose:" << takeoff_land.start_pose.transpose() << std::endl;
	...
	case CMD_CTRL:
	{
		...
		else if(cancle_mission_data.cancle_mission_flag)//接收到悬停指令，进入悬停模式
		{
			cancle_mission_data.cancle_mission_flag = false;
			cmd_data.have_cmd_data = false;
			state = AUTO_HOVER;
			set_hov_with_odom();
			des = get_hover_des();
			ROS_INFO("[px4ctrl] cancle_mission_flag is true,From CMD_CTRL(L3) to AUTO_HOVER(L2)!");
		}
		else
		{
			des = get_cmd_des();
		...
		if (takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::LAND)
		{
			...
			set_start_pose_for_takeoff_land(odom_data);
			std::cout << "takeoff start pose:" << takeoff_land.start_pose.transpose() << std::endl;
			ROS_INFO("\033[32m[px4ctrl] CMD_CTRL(L3) --> AUTO_LAND\033[32m");
		}
		break;
	}
	...
	{
		motor_low_speed = false;
		px4_state = PX4_STATE::TAKFOFF;

		if(!rc_data.is_hover_mode)//进入手动模式
		{
			cmd_data.have_cmd_data = false;
			state = MANUAL_CTRL;
			toggle_offboard_mode(false);
		...
		else if (!rc_data.is_command_mode)//进入悬停模式
		{
			state = AUTO_HOVER;
			set_hov_with_odom();
			des = get_hover_des();
			ROS_INFO("[px4ctrl] From AUTO_TAKEOFF to AUTO_HOVER(L2)!");
		}
		else if(cancle_mission_data.cancle_mission_flag)//接收到悬停指令，进入悬停模式
		{
			cancle_mission_data.cancle_mission_flag = false;
			...
			set_hov_with_odom();
			des = get_hover_des();
			ROS_INFO("[px4ctrl] cancle_mission_flag is true,From AUTO_TAKEOFF to AUTO_HOVER(L2)!");
		}
	...
	{
		motor_low_speed = false;
		px4_state = PX4_STATE::LANDING;//降落过程中；
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			state = MANUAL_CTRL;
			toggle_offboard_mode(false);
		...
		else if(cancle_mission_data.cancle_mission_flag)//接收到悬停指令，进入悬停模式
		{
			...
			set_hov_with_odom();
			des = get_hover_des();
			ROS_INFO("[px4ctrl] cancle_mission_flag is true,From AUTO_LAND to AUTO_HOVER(L2)!");
		}
		else if (!get_landed())
		{
			des = get_takeoff_land_des(-param.takeoff_land.speed);
		...
		{
			px4_state = PX4_STATE::LANDED;//已经降落到地上
			rotor_low_speed_during_land = true;
	...
	//pub px4 state
	quadrotor_msgs::Px4State px4_state_msg;
	px4_state_msg.seq = cmd_sequence;
	px4_state_msg.px4_state = int(px4_state);
	static int count = 0;
	count++;
	if(count > 100)
	{
		std::cout << "seq:" << cmd_sequence << ", px4_state:" << px4_state_str[int(px4_state)] << std::endl;
		count = 0;
	}

	px4_state_pub.publish(px4_state_msg);
	...
	if (rotor_low_speed_during_land) // used at the start of auto takeoff
	{
		...
		ROS_ERROR("where = %d",where);
		ROS_ERROR("where = %d",where);
		ROS_ERROR("where = %d",where);
		ROS_ERROR("where = %d",where);
	}
	else if(motor_low_speed)
	{
		u.q = imu_data.q;
	...
	else
	{
		...
		case 0:
			//kdkd 
			debug_msg = controller.update_alg0(des, odom_data, imu_data, u, bat_data.volt,land_flag);
...
void PX4CtrlFSM::land_detector(const State_t state, const Desired_State_t &des, const Odom_Data_t &odom)
{
	...
	else
	{
		...
		else if (C12_satisfy && is_last_C12_satisfy)
		{
			...
			{
				takeoff_land.landed = true;;
				std::cout << "1takeoff_land.landed = true" << std::endl;
				ROS_ERROR("----------------------------");
				ROS_ERROR("----------------------------");
				ROS_ERROR("----------------------------");
				ROS_ERROR("des.p(2):%f, odom.p(2):%f",des.p(2),odom.p(2));
				ROS_ERROR("----------------------------");
				ROS_ERROR("----------------------------");
				ROS_ERROR("----------------------------");
...
Desired_State_t PX4CtrlFSM::get_takeoff_land_des(const double speed)
{
	...
	// 	debug_msg.header.frame_id = "takeoff";
	// 	debug_msg.header.stamp = ros::Time::now();
	// 	debug_msg.des_p_x_land = des.p(0);
	// 	debug_msg.des_p_y_land = des.p(1);
	// 	debug_msg.des_p_z_land = des.p(2);
	// 	debug_pub.publish(debug_msg);
	// }
	// else
	// {
	// 	double delta_t = (now - last_time).toSec(); // speed > 0 means takeoff
	// 	des.v = Eigen::Vector3d(0, 0, 0);
	// 	static ros::Time start_hover;
	// 	static bool land_hover = false;
	// 	if((fabs(odom_data.p[0] - takeoff_land.start_pose.head<3>()[0]) < 0.025 && fabs(odom_data.p[1] - takeoff_land.start_pose.head<3>()[1]) < 0.025 && !land_hover) || (odom_data.p[2] < home_z) )
	// 	{
	// 		takeoff_land.start_pose.head<3>() = takeoff_land.start_pose.head<3>() + Eigen::Vector3d(0, 0, speed * delta_t);
	// 		des.v = Eigen::Vector3d(0, 0, speed);
	// 		start_hover = ros::Time::now();
	// 		// std::cout<<"kdA"<<std::endl;
	// 	}else
	// 	{
	// 		land_hover = true;
	// 		double hover_duration = (ros::Time::now() - start_hover).toSec();
	...
	// 	des.p = takeoff_land.start_pose.head<3>();
	// 	//kdkd
	// 	debug_msg.header.frame_id = "land";
	// 	debug_msg.header.stamp = ros::Time::now();
	// 	debug_msg.des_p_x_land = des.p(0);
	// 	debug_msg.des_p_y_land = des.p(1);
	// 	debug_msg.des_p_z_land = des.p(2);
	...
	debug_msg.header.frame_id = "takeoff_land";
	debug_msg.header.stamp = ros::Time::now();
	debug_msg.des_p_x_land = des.p(0);
	debug_msg.des_p_y_land = des.p(1);
	debug_msg.des_p_z_land = des.p(2);
...
void PX4CtrlFSM::Emergency_Kill()
{
	mavros_msgs::CommandLong emergency_srv;
	emergency_srv.request.broadcast = false;
	emergency_srv.request.command = 400;
	emergency_srv.request.param1 = 0;
	emergency_srv.request.param2 = 21196;
	emergency_srv.request.confirmation = true;
	if (reboot_FCU_srv.call(emergency_srv))
	{
		ROS_WARN("motor kill successfully!");
		// vehicle_armed = false;
	}
	else
	{
		ROS_ERROR("motor kill failed");
	}
}
...
void PX4CtrlFSM::set_start_pose_for_takeoff_land(const Odom_Data_t &odom)
{
	...
	last_time = takeoff_land.toggle_takeoff_land_time;
}
