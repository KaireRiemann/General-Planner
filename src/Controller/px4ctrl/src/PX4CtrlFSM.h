...
public:
	...
	//kdkd
	At_Home_Data_t at_home_data;
	Arm_data_t arm_data;
	Cancle_Misssion_data_t cancle_mission_data;
	int cmd_sequence;
	...
	ros::Publisher ui_back_home_pub_; //debug
	ros::Publisher go_home_pub_; //debug
	ros::Publisher tragger_pub_; //debug
	ros::Publisher takeoff_height_pub_; //debug
	ros::Publisher finish_inspection_pub_; //debug
	...
	//kdkd
	// quadrotor_msgs::Instruction tragger_msgl

	Eigen::Vector4d hover_pose;
	ros::Time last_set_hover_pose_time;
	ros::Time duration_time_hover_to_cmd;
	bool start_hover_to_cmd = false;
	enum State_t
	{
	...
	const std::vector<std::string> px4_state_str = {"ARMED","DIS_AEMED","TAKFOFF","HOVERED","FLYING","LANDING","LANDED"};
