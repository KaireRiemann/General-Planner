    ...
    bool land_flag)
{
  ...
  const Eigen::Vector3d pid_error_accelerations = computePIDErrorAcc(odom, des, param,land_flag);
    ...
    bool land_flag)
{
  ...
  if (land_flag)
  {
    Kp(0) = param.gain.Klandp0;
    Kp(1) = param.gain.Klandp1;
    Kv(0) = param.gain.Klandv0;
    Kv(1) = param.gain.Klandv1;
    // std::cout<<"land + i !!!"<<std::endl;
    Eigen::Vector3d pid_pos_p_term(x_pos_error,y_pos_error,z_pos_error);
    debug.pid_P_pos_term = pid_pos_p_term.norm();
    debug.pid_P_vel_term = (acc_error-pid_pos_p_term).norm();

    static double x_pos_error_I,y_pos_error_I,z_pos_error_I;
    double I_term_max = 2;
    x_pos_error_I += x_pos_error*Ip(0);
    y_pos_error_I += y_pos_error*Ip(1);
    z_pos_error_I += z_pos_error*Ip(2);
    Limit_I_term(x_pos_error_I,I_term_max);
    Limit_I_term(y_pos_error_I,I_term_max);
    Limit_I_term(z_pos_error_I,I_term_max);

    Eigen::Vector3d pid_I_pos_term(x_pos_error_I,y_pos_error_I,z_pos_error_I);
    acc_error += pid_I_pos_term;

    debug.pid_I_pos_term = pid_I_pos_term.norm();
  }
