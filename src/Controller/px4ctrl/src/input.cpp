...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        // std::cout<<"msg.channels"<<"["<<i<<"]"<<":"<<msg.channels[i]<<std::endl;
...
void Takeoff_Land_Data_t::feed(quadrotor_msgs::TakeoffLandConstPtr pMsg)
{
    ...
    ROS_WARN("takeoff signal rec!");
    msg = *pMsg;
    rcv_stamp = ros::Time::now();
    *seq = pMsg->seq;
    triggered = true;
    takeoff_land_cmd = pMsg->takeoff_land_cmd;
    takeoff_hight = pMsg-> takeoff_hight;
}

At_Home_Data_t::At_Home_Data_t()
{
    rcv_stamp = ros::Time(0);
}

void At_Home_Data_t::feed(std_msgs::BoolConstPtr pMsg)
{
    msg = *pMsg;
    rcv_stamp = ros::Time::now(); 
    at_home = pMsg->data;
    ROS_WARN("[px4ctrl] At Home !!!");
}

Arm_data_t::Arm_data_t()
{
    arm_cmd = 0;
    rcv_stamp = ros::Time(0);
    msg.seq = 0;
...
void Arm_data_t::feed(quadrotor_msgs::ArmCmdConstPtr pMsg)
{
    msg = *pMsg;
    arm_cmd = pMsg->arm_cmd ? 1: -1;
    *seq = pMsg->seq;
    rcv_stamp = ros::Time::now(); 
    ROS_WARN("[px4ctrl] receivecd arm cmd : %d !!!",arm_cmd);
}

Cancle_Misssion_data_t::Cancle_Misssion_data_t()
{
    cancle_mission_flag = false;
}

void Cancle_Misssion_data_t::feed(geometry_msgs::PointStampedConstPtr cancle_mission_msg)
{
    cancle_mission_flag = true;
    ROS_WARN("[px4ctrl] received cancle signal");
}
