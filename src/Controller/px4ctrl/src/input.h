...
#include <quadrotor_msgs/ArmCmd.h>
...
public:
  ...
  void setSeq(int* seq_ptr){seq = seq_ptr;}
};
//kdkd at home 
class At_Home_Data_t
{
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  bool at_home{false};
...
{
public:
  quadrotor_msgs::ArmCmd msg;
  int arm_cmd;
  ros::Time rcv_stamp;
  int* seq = nullptr;

  Arm_data_t();
  void feed(quadrotor_msgs::ArmCmdConstPtr pMsg);
  void setSeq(int* seq_ptr){seq = seq_ptr;}
};

class Cancle_Misssion_data_t
{
public:
  bool cancle_mission_flag ;
  Cancle_Misssion_data_t();
  void feed(geometry_msgs::PointStampedConstPtr cancle_mission_msg);
};
#endif