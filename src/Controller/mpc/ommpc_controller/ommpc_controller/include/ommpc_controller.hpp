...
public:
  ...
  void feed_from_quadrotor_msgs(const quadrotor_msgs::PolynomialTrajectoryConstPtr &pMsg)
  {
    const quadrotor_msgs::PolynomialTrajectory &traj = *pMsg;
    if ((traj.type & quadrotor_msgs::PolynomialTrajectory::EMER_STOP) != 0)
    {
      traj_queue.clear();
      exec_traj = -1;
      total_traj_start_time = ros::Time(0);
      total_traj_end_time = ros::Time(0);
      ROS_WARN("[MPCCtrl] Received EMER_STOP trajectory.");
      return;
    }

    if ((traj.type & quadrotor_msgs::PolynomialTrajectory::POSITION_TRAJ) == 0)
    {
      return;
    }
    ...
    if (traj.piece_num_pos == 0 || traj.order_pos < 1)
    {
      ROS_ERROR("[MPCCtrl] Invalid polynomial trajectory shape.");
      return;
    }

    const int piece_num = static_cast<int>(traj.piece_num_pos);
    const int order = static_cast<int>(traj.order_pos);
    const int coeff_num = order + 1;
    if (static_cast<int>(traj.time_pos.size()) < piece_num ||
        static_cast<int>(traj.coef_pos_x.size()) < piece_num * coeff_num ||
        static_cast<int>(traj.coef_pos_y.size()) < piece_num * coeff_num ||
        static_cast<int>(traj.coef_pos_z.size()) < piece_num * coeff_num)
    {
      ROS_ERROR("[MPCCtrl] Polynomial trajectory coefficient size mismatch.");
      return;
    }

    oneTraj_Data_t traj_data;
    traj_data.traj_start_time = traj.start_WT_pos;
    if (traj_data.traj_start_time.isZero())
    {
      traj_data.traj_start_time = traj.header.stamp.isZero() ? ros::Time::now() : traj.header.stamp;
    }

    double t_total = 0.0;
    traj_data.traj.clear();
    traj_data.traj.reserve(piece_num);
    for (int i = 0; i < piece_num; ++i)
    {
      const double duration = traj.time_pos[static_cast<std::size_t>(i)];
      if (duration <= 1.0e-6)
      {
        ROS_ERROR("[MPCCtrl] Invalid polynomial trajectory duration.");
        return;
      }
      t_total += duration;

      Eigen::MatrixXd piece_coef(3, coeff_num);
      for (int j = 0; j < coeff_num; ++j)
      {
        const int offset = i * coeff_num + j;
        piece_coef(0, j) = traj.coef_pos_x[static_cast<std::size_t>(offset)];
        piece_coef(1, j) = traj.coef_pos_y[static_cast<std::size_t>(offset)];
        piece_coef(2, j) = traj.coef_pos_z[static_cast<std::size_t>(offset)];
      }
      traj_data.traj.emplace_back(duration, piece_coef);
    }
    traj_data.traj_end_time = traj_data.traj_start_time + ros::Duration(t_total);

    if ((traj.type & quadrotor_msgs::PolynomialTrajectory::YAW_TRAJ) != 0 &&
        traj.piece_num_yaw > 0 && traj.order_yaw >= 1)
    {
      const int yaw_piece_num = static_cast<int>(traj.piece_num_yaw);
      const int yaw_order = static_cast<int>(traj.order_yaw);
      const int yaw_coeff_num = yaw_order + 1;
      if (static_cast<int>(traj.time_yaw.size()) >= yaw_piece_num &&
          static_cast<int>(traj.coef_yaw.size()) >= yaw_piece_num * yaw_coeff_num)
      {
        traj_data.yaw_traj.clear();
        traj_data.yaw_traj.reserve(yaw_piece_num);
        for (int i = 0; i < yaw_piece_num; ++i)
        {
          const double duration = traj.time_yaw[static_cast<std::size_t>(i)];
          if (duration <= 1.0e-6)
          {
            traj_data.yaw_traj.clear();
            break;
          }
          Eigen::MatrixXd yaw_coef(1, yaw_coeff_num);
          for (int j = 0; j < yaw_coeff_num; ++j)
          {
            yaw_coef(0, j) = traj.coef_yaw[static_cast<std::size_t>(i * yaw_coeff_num + j)];
          }
          traj_data.yaw_traj.emplace_back(duration, yaw_coef);
        }
      }
    }
    ...
    traj_queue.clear();
    traj_queue.push_back(traj_data);
    total_traj_start_time = traj_data.traj_start_time;
    total_traj_end_time = traj_data.traj_end_time;
    exec_traj = 1;
    ROS_INFO("[MPCCtrl] Loaded quadrotor_msgs polynomial trajectory id=%u, pieces=%d, duration=%.3f.",
             traj.trajectory_id,
             piece_num,
             t_total);
  }
...
{
  double step_T{0.01};
  double hover_percent{0.5};
  double Q_pos_xy{1000.0};
  double Q_pos_z{800.0};
  double Q_velocity{20.0};
  double Q_attitude_rp{40.0};
  double Q_attitude_yaw{40.0};
  double R_thrust{0.5};
  double R_pitchroll{1.2};
  double R_yaw{0.6};
  double state_cost_exponential{0.5};
  double input_cost_exponential{0.5};
  double max_bodyrate_xy{6.0};
  double max_bodyrate_z{4.0};
  double min_thrust{1.0};
  double max_thrust{30.0};

  bool use_fix_yaw{false};
  bool use_trajectory_ending_pos{true};

  bool use_ref_txt{false};
  std::string ref_filename{"/traj/circle.txt"};
  double ref_time_step{0.01};

  double takeoff_land_speed{0.2};
  double takeoff_height{0.8};
};
...
public:
              ...
              Controller_Output_t &u)
  {
    ...
    last_solve_time_ = (ros::WallTime::now() - start).toSec();
    ...
    if (!mpc_solved)
      return false;
    last_optimal_cost_ = solution.optimal_cost;
    ...
    timing_feedback_ = 0.9 * timing_feedback_ + 0.1 * last_solve_time_;
  ...
  double getLastOptimalCost() const
  {
    return last_optimal_cost_;
  }

  double getSuggestedHoverPercentage() const
  {
    return gravity_ / thr2acc;
  }

  void getDesiredStart(Eigen::VectorXd &x_des, Eigen::VectorXd &u_des)
  {
          ...
          const Odom_Data_t &odom)
  {
    ...
    const bool has_yaw_traj = yaw_traj.getPieceNum() > 0;
    double yaw_t_all = has_yaw_traj ? yaw_traj.getTotalDuration() - 1.0e-3 : 0.0;
	    double t = tstart;
    double yaw, yaw_dot;
    ...
    for (int i = 0; i < nstep; i++)
    {
        ...
        else if (has_yaw_traj)
        {
          ...
          yaw = yaw_traj.getPos(yaw_t)(0);
          yaw_dot = yaw_traj.getVel(yaw_t)(0);
        }
	        else // directly compute from tangent line of traj
        {
          if (i == 0) // reset last_yaw_
