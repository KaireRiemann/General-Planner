...
typedef message_filters::sync_policies::ApproximateTime<tracking_detector::BoundingBoxes, nav_msgs::Odometry>
    YoloOdomSyncPolicy;
...
void update_state_callback(const tracking_detector::BoundingBoxesConstPtr &bboxes_msg, const nav_msgs::OdometryConstPtr &odom_msg) {
  ...
  const tracking_detector::BoundingBox* yolo_bbox = nullptr;
  for (const auto& bbox : bboxes_msg->bounding_boxes) {
    if (bbox.Class == target_label_ &&
        (yolo_bbox == nullptr || bbox.probability > yolo_bbox->probability)) {
      yolo_bbox = &bbox;
    }
  }
  if (yolo_bbox == nullptr) {
    ROS_DEBUG_THROTTLE(1.0, "No detection matching target_label '%s'", target_label_.c_str());
    return;
  }
...
int main(int argc, char** argv) {
  ...
  nh.param<std::string>("target_label", target_label_, "car");

  message_filters::Subscriber<tracking_detector::BoundingBoxes> yolo_sub_;
