...
public:
    ...
    size_t lidar_msg_count_{0};
    size_t lidar_point_count_{0};
    LiDARType lidarType() const { return lidar_type_; }

    DataPreprocess(Params &params, bool load_image = true)
        : cloud_input_(new pcl::PointCloud<Common::Point>)
    {
        ...
        for (const rosbag::MessageInstance &m : view)
        {
            ...
            if (auto livox_custom_msg = m.instantiate<livox_ros_driver::CustomMsg>())
            {
                ...
                ++lidar_msg_count_;
                lidar_point_count_ += livox_custom_msg->point_num;
        ...
        ROS_INFO("Loaded %zu points from the rosbag (%zu LiDAR messages, %zu raw points).",
                 cloud_input_->size(), lidar_msg_count_, lidar_point_count_);
    }
};
