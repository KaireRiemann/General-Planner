...
class SerialHandover(object):
    def __init__(self):
        ...
        # Private param name used by the started binary, e.g. config_path or
        # interface_config_path (general_planner_release runtime wrapper).
        self.fsm_config_param = rospy.get_param(
            "~fsm_config_param", "config_path")
        # Extra rosrun args (space-separated), e.g. generated_config_path.
        self.fsm_extra_args = rospy.get_param("~fsm_extra_args", "")
    ...
    def _kill_nodes(self, names):
        ...
        for name in names:
            ...
            subprocess.call(
                ["bash", "-lc",
                 "pkill -f 'devel/lib/general_planner/%s' >/dev/null 2>&1 || true; "
                 "pkill -f 'general_planner_release/bin/%s' >/dev/null 2>&1 || true; "
                 "pkill -f '%s\\.bin' >/dev/null 2>&1 || true" % (
    ...
    def _start_fsm(self):
        ...
        if self.fsm_config_path:
            cmd.append("_%s:=%s" % (self.fsm_config_param, self.fsm_config_path))
        if self.fsm_extra_args:
            cmd.extend(shlex.split(self.fsm_extra_args))
