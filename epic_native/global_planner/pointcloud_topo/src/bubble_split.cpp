...
void TopoGraph::generateBubble(const Eigen::Vector3f &low_bd,
  ...
  if (!reg_map_idx2ptr_.count(region_idx)) {
    ROS_WARN_THROTTLE(1.0, " -- [EPIC Native] Skip bubble outside topo region map.");
    return;
  }
  ...
  const Eigen::Vector3f local_center = center - region_bd_min;
  pcl::PointXYZ searchPoint(local_center.x(), local_center.y(), local_center.z());
