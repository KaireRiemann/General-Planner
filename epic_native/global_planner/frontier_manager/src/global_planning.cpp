...
void FrontierManager::generateTSPViewpoints(Eigen::Vector3f&center,  vector<TopoNode::Ptr> &viewpoints) {
  ...
  int dormant_skip = 0;
  int transient_unreachable_seen = 0;
  ...
  vector<int> idx;
  for (size_t i = 0; i < distance_odom2cluster.size(); i++) {
  ...
  #pragma omp parallel for
  // clang-format on
  for (size_t i = 0; i < clusters_can_be_searched_.size(); i++) {
  ...
  vector<int> idx2;
  for (size_t i = 0; i < tsp_clusters.size(); i++) {
  ...
  ROS_INFO("[EPIC Frontier] clusters total=%zu considered=%zu dormant_skip=%d transient_unreachable_seen=%d reachable_after_topo=%zu selected=%zu viewpoints=%zu",
           cluster_list_.size(), revp_clusters_vec.size(), dormant_skip,
           transient_unreachable_seen, clusters_can_be_searched_.size(),
           tsp_clusters.size(), viewpoints.size());
