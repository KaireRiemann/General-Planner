...
void FrontierManager::init(ros::NodeHandle &nh, LIOInterface::Ptr &lio_interface,
  ...
  nh.param("ViewpointManager/topo_reachability_timeout",
           vpp_.topo_reachability_timeout_, 3.0e-4f);
  nh.param("ViewpointManager/reachability_only_raycast",
           vpp_.reachability_only_raycast_, true);
...
int FrontierManager::surface_pos2idx(const PointType &pt) {
  ...
  Sphere_PosToIndex(Eigen::Vector3f::Zero(), pt_lidar_frame,
                                      surface_idx);
  return std::max(0, std::min(99, surface_idx.x())) * 200 +
         std::max(0, std::min(199, surface_idx.y()));
}
...
void FrontierManager::project_pts_2_depth_image(PointVector &pts_vec,
  ...
  auto project_pt = [&](PointType &pt) {
    ...
    Sphere_PosToIndex(Eigen::Vector3f::Zero(), pt_lidar_frame,
                                        surface_idx);
    if (surface_idx.x() < 0 || surface_idx.x() >= 100 ||
        surface_idx.y() < 0 || surface_idx.y() >= 200) {
      return;
    }
    const int depth_idx = surface_idx.x() * 200 + surface_idx.y();
    if (depth_img[depth_idx] < 0 || depth_img[depth_idx] > dis) {
      depth_img[depth_idx] = dis;
    }
  };
  for (auto &pt : pts_vec)
    project_pt(pt);
...
void FrontierManager::initClusterViewpoints(ClusterInfo::Ptr &cluster) {
  ...
  if (vps_init.empty()) {
    ...
    ROS_WARN_THROTTLE(1.0,
                      "[EPIC Frontier] cluster %d sampled no safe viewpoint: origin=%zu clearance_reject=%d box_reject=%d region_reject=%d min_clearance=%.2f",
                      cluster->id_, origin_viewpoints_.size(), clearance_reject,
                      box_reject, region_reject, vpp_.min_clearance_);
    return;
  }
...
void FrontierManager::removeUnreachableViewpoints(
  ...
  ros::Time t1 = ros::Time::now();
  graph_->insertNodes(nodes2insert, vpp_.reachability_only_raycast_); // raycast-only is fast but can be too strict
  ros::Time t2 = ros::Time::now();
  ...
  for (int i = 0; i < nodes2insert.size(); i++) {
    ...
    if (!graph_->graphSearch(closest_node, nodes2insert[i], topo_path,
                             vpp_.topo_reachability_timeout_)) {
      vp_cluster_kept[i] = false;
    } else {
...
FrontierManager::Sphere_PosToIndex(const Eigen::Vector3f &lidar_center,
  ...
  id.setZero();
  if (!pos.allFinite() || !lidar_center.allFinite()) {
    return;
  }
  ...
  double cos_theta = (pos(2) - lidar_center(2)) / dis;
  cos_theta = std::max(-1.0, std::min(1.0, cos_theta));
  double theta_z = acos(cos_theta); // 以z为极轴的转角
  if (theta_z < 0)
  ...
  id.x() = std::max(0, std::min(99, id.x()));
  id.y() = std::max(0, std::min(199, id.y()));
}
