...
BackupTrajOpt::optimize(const Trajectory &exp_traj,
    ...
    if (success && std::isinf(optimize(out_traj, cfg_.opt_accuracy))) {
        std::cout << YELLOW << " -- [GeneralPlanner] Minco backup_traj opt failed." << RESET << std::endl;
        success = false;
    }

    if (opt_vars.penalty_log(1) > cfg_.penna_pos * 0.05) {
        std::cout << YELLOW << " -- [GeneralPlanner] Minco backup_traj out of corridor." << RESET << std::endl;
        success = false;
    }
    out_ts = opt_vars.ts;
