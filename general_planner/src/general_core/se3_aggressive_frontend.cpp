#include "general_core/se3_aggressive_frontend.hpp"
...
void appendUnique(const super_utils::Vec3f &point,
...
bool SE3AggressiveFrontend::buildGuidePath(
  ...
  if (astar_ && map_manager_ && map_manager_->ready()) {
    ...
    if ((ret == super_utils::REACH_GOAL || ret == super_utils::REACH_HORIZON) &&
        !astar_path.empty()) {
...
bool SE3AggressiveFrontend::buildCorridor(
    const super_utils::vec_E<super_utils::Vec3f> &guide_path,
  ...
  super_utils::Vec3f shifted_start = guide_path.front();
...
bool SE3AggressiveFrontend::buildProblem(const super_utils::StatePVAJ &head,
